...
contract Game {
    ...
    function moveBlock(
        ...
        // check cooldown
        GameTypes.BlockData memory _startTileBlockData = utils
            ._getWorldBlockData(startTile.worldBlockId);
        GameTypes.ItemWithMetadata memory _startBlockMetadata = utils._getItem(
            _startTileBlockData.blockId
        );

        require(
            block.timestamp - _startTileBlockData.lastMoved >=
                _startBlockMetadata.moveCooldown,
            "engine/move-cooldown"
        );
