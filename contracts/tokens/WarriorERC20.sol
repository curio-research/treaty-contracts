...
contract WarriorERC20 is ERC20 {
    ...
    function transferFrom(
        ...
        if (recipientMaxLoad == 0 || recipientCurrentBalance + _amount <= recipientMaxLoad) {
            admin.updateInventoryAmount(_from, "Warrior", _amount, false);
            admin.updateInventoryAmount(_to, "Warrior", _amount, true);
            emit Transfer(_from, _to, _amount);
            return true;
        } else {
            admin.updateInventoryAmount(_from, "Warrior", recipientMaxLoad - recipientCurrentBalance, false);
            admin.updateInventoryAmount(_to, "Warrior", recipientMaxLoad - recipientCurrentBalance, true);
    ...
    function transferAll(address _from, address _to) public onlyGame returns (bool) {
        uint256 amount = getter.getInventoryBalance(_from, "Warrior");
        // note: copy paste erc20 standard lines;
        // todo: should these data be stored in ECS ???
        uint256 allowed = allowance[_from][msg.sender]; // Saves gas for limited approvals.
        ...
        if (recipientMaxLoad == 0 || recipientCurrentBalance + amount <= recipientMaxLoad) {
            admin.updateInventoryAmount(_from, "Warrior", amount, false);
            admin.updateInventoryAmount(_to, "Warrior", amount, true);
    ...
    function dripToken(address _address, uint256 _amount) public onlyGame {
        ...
        if (recipientMaxLoad == 0 || recipientCurrentBalance + _amount <= recipientMaxLoad) {
            admin.updateInventoryAmount(_address, "Warrior", _amount, true);
            emit Transfer(address(0), _address, _amount);
        } else {
            admin.updateInventoryAmount(_address, "Warrior", recipientMaxLoad - recipientCurrentBalance, true);
            emit Transfer(address(0), _address, _amount);
        }
    }
    ...
    function destroyToken(address _address, uint256 _amount) public onlyGame {
        // note: copy paste erc20 standard lines;
        totalSupply -= _amount;
        admin.updateInventoryAmount(_address, "Warrior", _amount, false);
        emit Transfer(address(0), _address, _amount);
    }
}
