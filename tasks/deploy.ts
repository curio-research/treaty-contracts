  ...
  .setAction(async (args: any, hre: HardhatRuntimeEnvironment) => {
    ...
    if (isDev) {
      ...
      if (existingDeployments.length == 0) {
        await fsPromise.writeFile(configFileDir, "[" + JSON.stringify(configFile) + "]");
      } else {
        await fsPromise.writeFile(configFileDir, "[" + existingDeployments.substring(1, existingDeployments.length - 2) + "," + JSON.stringify(configFile) + "]");
      }
