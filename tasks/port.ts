...
task("port", "compile and port contracts over to frontend repo").setAction(
  async (args: HardhatArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    const gameAbi = JSON.stringify(
      (await hre.artifacts.readArtifact("Game")).abi
    );
    const gettersAbi = JSON.stringify(
      (await hre.artifacts.readArtifact("Getters")).abi
    );
    ...
    await fsPromise.writeFile(
      path.join(clientAbiDir, "Getters.json"),
      gettersAbi
    );

    // save typechain files
    const clientTypechainDir = path.join(
      __dirname,
      "../../frontend/src/network/typechain-types"
    );
    ...
    const configClientDir = path.join(
      __dirname,
      "../../frontend/src/game.config.json"
    );
