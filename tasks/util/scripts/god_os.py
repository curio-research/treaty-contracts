...
def get_hourly_gather_rate_per_army(resource_type: Resource) -> int:
    ...
    Crystal/Food mints from gather are both medium
    """
    ...
    building_type = None
    if (resource_type == Resource.CRYSTAL):
        building_type = Building.CRYSTALMINE
    if (resource_type == Resource.FOOD):
        building_type = Building.FARM
    (crystal_hourly_yield, food_hourly_yield) = get_building_hourly_yield_by_level(
        corresponding_building_level, building_type)

    projected_crystalmine_count = expected_crystal_density() * get_capital_tiles_interval() * \
        math.ceil((game_instance.max_capital_level + 1) / 2)
    projected_farm_count = expected_farm_density() * get_capital_tiles_interval() * \
        math.ceil((game_instance.max_capital_level + 1) / 2)

    crystal_gather_rate = crystal_hourly_yield * projected_crystalmine_count / \
    ...
    if (resource_type == Resource.CRYSTAL):
        return crystal_gather_rate * game_instance.troop_gathering_bonus
    if (resource_type == Resource.FOOD):
...
def resource_cap_per_troop() -> int:
    ...
    (building_crystal_cap, building_food_cap) = get_building_resource_cap(corresponding_building_level,
                                                                          Building.CRYSTALMINE) + get_building_resource_cap(corresponding_building_level, Building.FARM)
    median_army_size = get_troop_size_by_center_level(
        math.ceil((game_instance.max_capital_level + 1) / 2))
    # no difference btw resources, so take the avg
    return (building_crystal_cap + building_food_cap) / 2 / (median_army_size) * game_instance.troop_cap_to_buildings_cap_ratio
...
def get_building_resource_cap(level: int, buildingType: Building) -> np.array:
    """
    Growth: Slow Exponential
    Gist: crystalCap, foodCap = playerLoginIntervalInMinutes / 60 * buildingHourlyYields
    """
...
def get_building_base_hourly_yield(building_type: Building) -> np.array:
    ...
    crystal_cost_per_troop = game_instance.resource_weight_light
    food_cost_per_troop = game_instance.resource_weight_heavy
    total_crystalcost = total_troop * crystal_cost_per_troop
    ...
    # Crystal Mint: Harvest (low), Crystal Burn: Troop (light); also need to consider density level
    base_crystal_food_hourly_yield = total_crystalcost / total_seconds * 3600 * (game_instance.resource_weight_low/game_instance.resource_weight_light) \
        / (game_instance.init_player_crystalmine_count)

    if building_type == Building.FARM:
        return np.array([0, base_farm_food_hourly_yield])
    elif building_type == Building.CRYSTALMINE:
        return np.array([base_crystal_food_hourly_yield, 0])
    elif building_type == Building.CAPITAL:
        return np.array([base_crystal_food_hourly_yield, base_farm_food_hourly_yield])
    else:
        return np.array([0, 0])
...
def get_building_hourly_yield_by_level(level: int, building_type: Building) -> np.array:
    ...
    crystal_base_hourly_yield = hourly_yield[0]
    food_base_hourly_yield = hourly_yield[1]
    crystal_hourly_yield = math.floor(logarithmic_curve(game_instance.max_capital_level *
                                                        game_instance.capital_level_to_building_level)(level) / logarithmic_curve(9)(1) * crystal_base_hourly_yield)
    ...
    return np.array([crystal_hourly_yield, food_hourly_yield])
...
def get_barbarian_reward(level: int) -> np.array:
    ...
    return np.array([crystal_reward, food_reward])
...
def get_tile_upgrade_cost(level: int) -> np.array:
    ...
    foodcost_per_troop = game_instance.resource_weight_heavy * \
        game_instance.tile_troop_discount
    total_crystalcost = (get_tile_troop_count(level + 1) -
                         get_tile_troop_count(level - 1)) * crystalcost_per_troop
    total_foodcost = (get_tile_troop_count(level + 1) -
                      get_tile_troop_count(level - 1)) * foodcost_per_troop
    return np.array([total_crystalcost, total_foodcost])
...
def get_building_upgrade_cost(level: int, building_type: Building) -> np.array:
    ...
    """
    # cost is easy to calculate for crystalmine (cost curve faster than food)
    crystalmine_crystalcost = crystal_payback_period_curve_in_hour(game_instance.max_capital_level * game_instance.capital_level_to_building_level)(level) \
        * get_building_hourly_yield_by_level(level, Building.CRYSTALMINE)[0]
    # calculate foodcost based on resource weight; NOTE: not scientific, a leap of faith here
    crystalmine_foodcost = crystalmine_crystalcost / \
        game_instance.resource_weight_heavy * game_instance.resource_weight_low
    # assumption is that player spend crystal equivalently on two types of building
    # FIXME: 3 is hardcoded for now; this is technically not right
    farm_crystalcost = crystalmine_crystalcost * game_instance.init_player_crystalmine_count / \
    ...
    farm_foodcost = farm_foodcost_raw * game_instance.resource_weight_low / \
        (game_instance.resource_weight_heavy + game_instance.resource_weight_low)
    if building_type == Building.CRYSTALMINE:
        return np.array([crystalmine_crystalcost, crystalmine_foodcost])
    if building_type == Building.FARM:
        return np.array([farm_crystalcost, farm_foodcost])
    if building_type == Building.CAPITAL:
        ...
        unlocked_crystalmine_count = expected_crystal_density() * \
            get_capital_tiles_interval()
        # unlocked_farm_count = expected_farm_density() * get_capital_tiles_interval()
        # here I choose to use the new capital level as base
        corresponding_building_level = building_level_based_on_center_level(
            level + 1)
        expected_crystalmine_hourly_yield = get_building_hourly_yield_by_level(
            corresponding_building_level, Building.CRYSTALMINE)[0]
        # expected_farm_hourly_yield = get_building_hourly_yield_by_level(
        #     corresponding_building_level, Building.FARM)[1]

        tax_crystal = unlocked_crystalmine_count * expected_crystalmine_hourly_yield * crystal_payback_period_curve_in_hour(
        ...
        # FIXME: tax_crystal hardcoded to * 10
        return np.array([crystalmine_crystalcost + farm_crystalcost + game_instance.capital_crystal_tax_factor * tax_crystal, crystalmine_foodcost + farm_foodcost])


def get_move_capital_cost(level: int) -> np.array:
    (crystal_upgrade_cost, food_upgrade_cost) = get_building_upgrade_cost(
        level, Building.CAPITAL)
    return np.array([crystal_upgrade_cost * game_instance.move_capital_to_upgrade_cost_ratio, food_upgrade_cost * game_instance.move_capital_to_upgrade_cost_ratio])
...
def expected_crystal_density() -> float:
    return 0.13


def expected_farm_density() -> float:
    return 1 - expected_crystal_density()
...
def crystal_payback_period_curve_in_hour(max_level: int) -> LambdaType:
    """
    Growth: slow exponential
    Note: sum from f(1) to f(9) is 19.757. Leap of faith: assume this is reasonable for 72 hr gameplay
    Note: max_level not only affects x value but also total gameplay time -> need to make sure sum of y(x) is below 1/3 of all upgrades
...
class Game:
    ...
    init_player_crystalmine_count = 1
    """
    How many crystalmines avg players have when capital level is 1
    Determine resource density. Note that one of the crystalmines is capital
    """
    ...
    def __init__(self, mode: GameMode) -> None:
        if mode == GameMode.THREE_PLAYER_SHORT_TEST:
            ...
            self.init_player_crystalmine_count = 1
            ...
            self.capital_crystal_tax_factor = 30
        ...
        if mode == GameMode.PUBLIC_LAUNCH:
            ...
            self.init_player_tile_count = 3
            self.expected_play_time_in_hour = 144
            self.upgrade_time_to_expected_play_time_ratio = 0.3
            self.init_player_crystalmine_count = 1
            self.init_player_farm_count = 1
            ...
            self.capital_crystal_tax_factor = 25
    """
    Constant format:
    "subject": "Farm",
    "object": "Food",
    ...
    def export_json_parameters(self) -> None:
        ...
        game_parameters.append({"subject": "Army", "componentName": "Rate", "object": "Crystal", "level": 0,
                               "functionName": "gather", "value": int(get_hourly_gather_rate_per_army(Resource.CRYSTAL) * 1000 / 3600)})
        ...
        game_parameters.append({"subject": "Troop Production", "componentName": "Cost", "object": "Crystal",
        ...
        game_parameters.append({"subject": "Capital", "componentName": "Amount", "object": "Crystal",
        ...
        # Building Stats
        for bt in [Building.CRYSTALMINE, Building.FARM, Building.CAPITAL]:
            max_building_level = 1
            building_type = bt
            if bt == Building.CRYSTALMINE:
            ...
            while curr_level <= max_building_level:
                ...
                (crystal_hourly_yield, food_hourly_yield) = get_building_hourly_yield_by_level(
                    curr_level, building_type)
                (crystal_cap, food_cap) = get_building_resource_cap(
                    curr_level, building_type)
                game_parameters.append({"subject": building_type, "componentName": "Yield", "object": "Crystal",
                                       "level": curr_level, "functionName": "", "value": int(crystal_hourly_yield * 1000 / 3600)})
                ...
                game_parameters.append({"subject": building_type, "componentName": "Load", "object": "Crystal",
                                       "level": curr_level, "functionName": "", "value": int(crystal_cap * 1000)})
                ...
                if curr_level <= max_building_level:
                    game_parameters.append({"subject": building_type, "componentName": "Cost", "object": "Crystal",
                                           "level": curr_level, "functionName": "Upgrade", "value": int(crystal_upgrade_cost * 1000)})
                ...
                if building_type == Building.CAPITAL:
                    ...
                    (move_cost_crystal, move_cost_food) = get_move_capital_cost(
                        curr_level)
                    game_parameters.append({"subject": "Capital", "componentName": "Cost", "object": "Crystal",
                                           "level": curr_level, "functionName": "Move", "value": int(move_cost_crystal) * 1000})  # FIXME: hardcoded, attention @Modeo
        ...
        while curr_level <= self.max_capital_level * self.capital_level_to_building_level:
            (reward_crystal, reward_food) = get_barbarian_reward(curr_level)
            barbarian_count = get_barbarian_count_by_level(curr_level)
            game_parameters.append({"subject": "Barbarian", "componentName": "Reward", "object": "Crystal",
                                   "level": curr_level, "functionName": "", "value": int(reward_crystal * 1000)})
        ...
        while curr_level <= max_tile_level:
            ...
            if curr_level != max_tile_level:
                (cost_crystal, cost_food) = get_tile_upgrade_cost(curr_level)
                game_parameters.append({"subject": "Tile", "componentName": "Cost", "object": "Crystal",
                                       "level": curr_level, "functionName": "Upgrade", "value": int(cost_crystal * 1000)})
    ...
    def export_excel_parameters(self):
        ...
        tile_stats = pd.DataFrame(columns=[
                                  'Level', 'Cooldown(s): Upgrade', 'Cost: Crystal', 'Cost: Food', 'Amount: Guard'])
        ...
        while curr_level <= max_tile_level:
            ...
            tile_stats = tile_stats.append({
                ...
                'Cost: Crystal': int(cost_crystal * 1000),
        ...
        capital_stats = pd.DataFrame(columns=['Level', 'Cooldown(s): Upgrade', 'Upgrade Cost: Crystal', 'Upgrade Cost: Food', 'Cooldown(s): moveCapital',
                                     'Move Cost: Crystal', 'Move Cost: Food', 'Yield(s): Crystal', 'Yield(s): Food', 'Cap: Food', 'Cap: Crystal', 'Army Size', 'Tile Count'])
        crystalmine_stats = pd.DataFrame(columns=[
            'Level', 'Cooldown(s): Upgrade', 'Upgrade Cost: Crystal', 'Upgrade Cost: Food', 'Yield(s): Crystal', 'Cap: Crystal'])
        farm_stats = pd.DataFrame(columns=['Level', 'Cooldown(s): Upgrade',
                                  'Upgrade Cost: Crystal', 'Upgrade Cost: Food', 'Yield(s): Food', 'Cap: Food'])
        ...
        payback_stats = pd.DataFrame(
            columns=['Level', 'Crystalmine PB (hr)', 'Farm PB (hr)'])
        ...
        while curr_level < max_building_level:
            level = 'lv' + str(curr_level)
            (crystalmine_crystal_upgrade_cost, a) = get_building_upgrade_cost(
                curr_level, Building.CRYSTALMINE)
            (a, farm_food_upgrade_cost) = get_building_upgrade_cost(
                curr_level, Building.FARM)

            (crystalmine_crystal_nextlv_yield, a) = get_building_hourly_yield_by_level(
                curr_level + 1, Building.CRYSTALMINE)
            (a, farm_food_nextlv_yield) = get_building_hourly_yield_by_level(
                curr_level + 1, Building.FARM)

            mine_pay_back_hrs = crystalmine_crystal_upgrade_cost / \
                crystalmine_crystal_nextlv_yield
            farm_pay_back_hrs = farm_food_upgrade_cost / farm_food_nextlv_yield

            payback_stats = payback_stats.append({
                'Level': level,
                'Crystalmine PB (hr)': mine_pay_back_hrs,
        ...
        gather_stats = pd.DataFrame(columns=['Capital Level', 'Army Size', '# Gatherings to Upgrade',
                                    'Crystal Gathering Time (s)', 'Food Gathering Time (s)', '# (hr) Maximal Troop Production'])
        ...
        while curr_level < self.max_capital_level:
            ...
            # only consider crystal cost
            (crystal_upgrade_cost, food_upgrade_cost) = get_building_upgrade_cost(
                curr_level, Building.CAPITAL)

            times_gathering_to_upgrade = crystal_upgrade_cost / gathering_capacity

            tile_count_limit += get_capital_tiles_interval()
            projected_farm_count = expected_farm_density() * tile_count_limit
            (crystal_yield, food_yield) = get_building_hourly_yield_by_level(
            ...
            crystal_gathering_rate = get_hourly_gather_rate_per_army(
                Resource.CRYSTAL)
            ...
            gather_stats = gather_stats.append({
                ...
                'Crystal Gathering Time (s)': gathering_capacity / crystal_gathering_rate * 3600,
        ...
        barbarian_stats = pd.DataFrame(
            columns=['Capital Level', '# Defeats to Upgrade', '# Full Harvest to Upgrade'])
        # Note: barbarians are always 4 or 8 levels in this version
        (reward_crystal, reward_food) = get_barbarian_reward(6)
        ...
        while curr_level < self.max_capital_level:
            ...
            barbarian_stats = barbarian_stats.append({
                'Capital Level': level,
                '# Defeats to Upgrade': crystal_upgrade_cost / reward_crystal,
                '# Full Harvest to Upgrade': crystal_upgrade_cost / get_building_resource_cap(curr_level*3, Building.CRYSTALMINE)[0]
            }, ignore_index=True)
        ...
        crystalmine_stats.to_excel(
            writer, sheet_name='Crystal Mine', index=False)
