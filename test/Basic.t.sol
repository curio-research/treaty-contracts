...
contract BasicTest is Test, DiamondDeployTest {
    ...
    function testInitializePlayer() public {
        ...
        assertEq(_player1.goldBalance, 20);
        assertEq(_player1.totalGoldGenerationPerUpdate, 5);
        assertEq(_player1.totalOilConsumptionPerUpdate, 0);
    ...
    function testUpdatePlayerBalance() public {
        ...
        assertEq(getter.getPlayer(player1).goldBalance, 25);
        assertEq(getter.getPlayer(player1).oilBalance, 20);
        ...
        assertEq(getter.getPlayer(player1).totalOilConsumptionPerUpdate, 3);
        ...
        assertEq(getter.getPlayer(player1).totalGoldGenerationPerUpdate, 5);
        assertEq(getter.getPlayer(player1).totalOilConsumptionPerUpdate, 6);
        ...
        assertEq(getter.getPlayer(player1).oilBalance, 14);
        ...
        assertEq(getter.getPlayer(player1).totalOilConsumptionPerUpdate, 8);
        vm.stopPrank();

        vm.warp(4);
        helper.updatePlayerBalances(player1);
        assertEq(getter.getPlayer(player1).goldBalance, 40);
        assertEq(getter.getPlayer(player1).oilBalance, 6);
    }
    ...
    function testPauseGame() public {
        ...
        helper.spawnTroop(Position({x: 3, y: 6}), player1, infantryTroopTypeId); // no consumption
        vm.stopPrank();

        // verify initial states
        Player memory _player1Data = getter.getPlayer(player1);
        assertEq(block.timestamp, 1);
        assertEq(_player1Data.numOwnedBases, 2);
        assertEq(_player1Data.numOwnedTroops, 3);
        assertEq(_player1Data.totalGoldGenerationPerUpdate, 5);
        assertEq(_player1Data.totalOilGenerationPerUpdate, 5);
        assertEq(_player1Data.totalOilConsumptionPerUpdate, 2);
        assertEq(_player1Data.goldBalance, 20);
        assertEq(_player1Data.oilBalance, 20);
        ...
        assertEq(getter.getPlayer(player1).goldBalance, 40); // 20 + 5 * 4
        assertEq(getter.getPlayer(player1).oilBalance, 32); // 20 + (5-2) * 4

        // pause game after another 2 seconds
        vm.warp(7);
        assertEq(getter.getPlayer(player1).oilBalance, 32);
        vm.prank(deployer);
        helper.pauseGame();

        // verify player balance update
        assertEq(getter.getPlayer(player1).oilBalance, 38); // 32 + (5-2) * 2;
        ...
        vm.warp(10);
        assertEq(getter.getPlayer(player1).oilBalance, 38);
        vm.prank(deployer);
        helper.resumeGame();
        ...
        assertEq(_player1Data.oilBalance, 41); // 38 + (5-2)
        assertEq(_player1Data.numOwnedTroops, 3);
    }
}
